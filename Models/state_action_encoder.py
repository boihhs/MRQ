...
class State_Action_Encoder:
    ...
    def __init__(self, action_dim, za_dim, zsa_dim, out_dim, num_bins, key=random.PRNGKey(0)):
        self.action_dim, self.za_dim, self.zsa_dim, self.out_dim, self.num_bins = action_dim, za_dim, zsa_dim, out_dim, num_bins
        self.key = key
        ...
        self.w2, self.b2, self.key = self._layer(action_dim + za_dim,     512, self.key, bias_scale=0.0, use_normal=True)
        self.w3, self.b3, self.key = self._layer(512,     512, self.key, bias_scale=0.0, use_normal=True)
        self.w4, self.b4, self.key = self._layer(512,     512, self.key, bias_scale=0.0, use_normal=True)
    ...
    def __call__(self, zs, action):
        ...
        zsa = jnp.concatnate([zs, za], axis= -1)
    ...
    def tree_flatten(self):
        ...
        aux = (self.action_dim, self.za_dim, self.zsa_dim, self.out_dim, self.num_bins)
        return children, aux
    ...
    def tree_unflatten(cls, aux, children):
        ...
        (obj.action_dim, obj.za_dim, obj.zsa_dim, obj.out_dim, obj.num_bins) = aux
